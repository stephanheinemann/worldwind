...
//	Random rand = new Random();
	...
	/** maps each action to the correspondent Q value for this state */
	protected Map<Action, Double> line = new TreeMap<>();
...
//	}
	/** Constructs a Q-table entry, with a map of Q values to actions for 
	 * a state
	 * 
	 * @param the corresponding state
	 */
	public QLine(State state) {
		this.setState(state);
	}
...
//	}
	...
	public void initQValues() {
		for (int i = 0; i < Action.values().length; i++) {
			this.line.put(Action.values()[i], 0d);
		}
	}
...
//	}
	 ...
	 * @param the Q-value
	 */
	public void updateQValue(Action action, double qValue) {
		this.line.put(action, qValue);
	}
...
//}
	...
	public Action getMaxQValue() {
		double maxQValue = -500000;
		Action max = null;
		for (Action action : line.keySet()) {
			if (line.get(action) > maxQValue) {
				maxQValue = line.get(action);
				max = action;
			}
			else if (line.get(action) == maxQValue) {
				 double r = rand.nextDouble();
				 if (r>0.5) max = action;
			}
		}
	...
	/** Gets action index correspondent to the i'th max Q-value. For example, i=0
	 * returns max qValue, while i=1 returns second max qValue, and so on
	 * 
	 * @return action index correspondent to the i'th max Q-value
	 */
	public Action getMaxQValue(int i) {
		Action orderedActions[] = Action.values();
		for (int m=0; m<orderedActions.length-1; m++) {
			for (int n=0; n<orderedActions.length-m-1; n++) {
				if(line.get(orderedActions[n]) > line.get(orderedActions[n+1])) {
					Action temp = orderedActions[n];
					orderedActions[n] = orderedActions[n+1];
					orderedActions[n+1] = temp;
				}
			}
		}
		return orderedActions[orderedActions.length-i];
	}
	
	/** Gets action index following epsilon greedy policy. Random in case of tie.
	 * 
	 * @return action index correspondent to the max Q-value
	 */
	public Action getEGreedyAction(double epsilon) {
		Action a = null;
		double r = rand.nextDouble();
		if (r<epsilon) {
			a = Action.getRandom();
